...
/**
 * Specialized manager for After-H game modes.
 */
class ModeManager extends ContentManager {
  constructor(discord, database, screenshotService, domain) {
    super(discord, database, screenshotService, domain, "modes");
  }
  ...
  async fetchDataFromAPI(callback) {
    await this._database.fetchNewContent("modes", callback);
  }
  ...
  async refreshServer(server, items, baseUrls, i18nFunction, interaction) {
    ...
    if (WEAPONS_CHANNELS.length === 0) {
      console.error(`No mode channel found in the "${server.name}" server.`);
      return;
    }
