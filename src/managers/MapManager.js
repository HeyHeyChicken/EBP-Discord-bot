...
class MapManager extends ContentManager {
  constructor(discord, database, screenshotService, domain) {
    super(discord, database, screenshotService, domain, "maps");
  }
  ...
  async fetchDataFromAPI(callback) {
    await this._database.fetchNewContent("maps", callback);
  }
  ...
  async refreshServer(server, items, baseUrls, i18nFunction, interaction) {
    ...
    if (WEAPONS_CHANNELS.length === 0) {
      console.error(`No map channel found in the "${server.name}" server.`);
      return;
    }
